...
from .data_preparation import create_N_way_few_shot_dataset, split_dataset,DomainDataset
...
def startExperiment(transform,data_folder,train_domains,test_domains, embedding_model_name,embedding_dim,N,num_tasks,num_epochs,type_of_network):
    ...
    for shots in [1,5,10,20]:
        ...
        if type_of_network == "MatchingNetworks":
            model = MatchingNetworks(backbone=dino_vit_embedding, feature_dimension=embedding_dim).to(device)
        elif type_of_network == "PrototypicalNetworks":
            model = PrototypicalNetworks(backbone=dino_vit_embedding).to(device)
        else:
            print("Invalid model type_of_network")

        for domain in train_domains:
            print("Training Domain:", domain)

            domain_path = os.path.join(data_folder, domain)
            dataset = DomainDataset(domain_path, transform)

            for task in range(num_tasks):

                # Create N-way few-shot datasets
                train_support_set, train_query_set = create_N_way_few_shot_dataset(dataset, shots, N)
                ...
                # Train the model
                best_model_state,best_training_loss = train(model, train_support_loader, train_query_loader, num_epochs,device)
        ...
        # Tesing episodes
            for task in range(num_tasks):
                ...
                # Create N-way few-shot datasets
                test_support_set,test_query_set = create_N_way_few_shot_dataset(dataset, shots, N)
                ...
                # Test the model
                task_accuracy = testTheModel(test_support_loader,test_query_loader, model,device)
        ...
        print(f"Cross-Domain. 5-way {shots}-shot learning with {num_tasks} training tasks and {num_epochs} epochs for each task: Mean accuracy: {mean_accuracy:.2f}%, Standard Deviation: {std_accuracy:.2f}%, 95% CI: {confidence_interval:.2f}%")
        print("---------------------------------------------------------")
